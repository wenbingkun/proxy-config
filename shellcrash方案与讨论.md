...
> 状态：代码、CI 与首次路由器真实运行验收已完成，仍待路由器重启恢复和真实回滚演练。本文从本节起为当前结论；下方“原始讨论”仅保留审计上下文，不是现行操作指南。
...
| 全新安装由 ShellCrash 获取核心 | 已实现且已验证 | 目标路由器首次单订阅部署成功，ShellCrash 自动下载 Mihomo 核心、Country.mmdb 和 ruleset 并启动服务 |
| 米家兼容 | 已实现且已验证 | 米家 App 刷新、设备详情和设备操作均正常，未需添加未经证实的广泛域名清单 |
| IPv4、IPv6、启动和重启 | 部分完成 | IPv4/IPv6 国内解析、国外出口和透明分流已验证，未发现本地 IPv6 直连绕行；路由器重启后自动恢复仍待验证 |
...
- [x] 使用单订阅执行首次部署，ShellCrash 已自动取得核心并启动服务。
- [x] 国内、国外、Steam 下载、广告域名和米家设备通信已冒烟验证，连接日志也已核对。Steam 后续按用户决定合并至 `🎮 游戏平台`。
- [x] IPv4 与 IPv6 公网出口均已检查，未发现未处置的本地 IPv6 直连绕行。
