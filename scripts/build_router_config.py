...
WINDOWS_SINGLE_OUTPUT_PATH = ROOT / "clash" / "config-single.yaml"
ROUTER_DUAL_OUTPUT_PATH = ROOT / "clash" / "config-router.template.yaml"
ROUTER_SINGLE_OUTPUT_PATH = ROOT / "clash" / "config-router-single.template.yaml"
...
def keep_proxy_providers(config: dict[str, object], provider_names: set[str]) -> None:
    providers = config.get("proxy-providers")
    ...
    def prune_uses(value: object) -> None:
        if isinstance(value, dict):
            use = value.get("use")
            if isinstance(use, list):
                value["use"] = [name for name in use if name in provider_names]
...
def dump_config(config: dict[str, object]) -> str:
    rendered = yaml.dump(
        config,
    ...
    if re.search(r"(?m)^\s*(?:<<:|[*&][A-Za-z0-9_-]+)", rendered):
        raise ValueError("generated config still contains YAML anchors or aliases")
    return rendered


def build_windows_single_config() -> str:
    output = load_source()
    provider_names = {"Sub"}
    keep_proxy_providers(output, provider_names)
    validate_references(output, provider_names)

    rendered = dump_config(output)
    if "Sub2" in rendered:
        raise ValueError("generated Windows single-subscription config still references Sub2")
    header = (
        "# Repository artifact generated by scripts/build_router_config.py. Do not edit it by hand.\n"
        "# Source of truth: clash/config.yaml\n"
        "# Windows single-subscription variant: only proxy provider Sub is retained.\n\n"
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate the Windows single-subscription config and ShellCrash policy templates."
    )
    ...
    try:
        outputs = {
            WINDOWS_SINGLE_OUTPUT_PATH: build_windows_single_config(),
            ROUTER_DUAL_OUTPUT_PATH: build_router_config(2),
            ROUTER_SINGLE_OUTPUT_PATH: build_router_config(1),
        }
