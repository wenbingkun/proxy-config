...
> **注记（2026-09-22 追加）：** 2026-09-22 只读复核时，`🐟 兜底分流` 的选择为 `♻️ 自动选择`，当时实际出口为香港节点。本文中出现的 `DIRECT` 是 2026-09-17 的历史快照；该选择变更的时间和原因未核实，不应用这份快照解释后续流量。本注记同样适用于第 7 节中引用 `兜底分流 = DIRECT` 的段落。
